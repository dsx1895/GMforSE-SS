...
class System(pl.LightningModule):
    ...
    def common_step(self, batch, batch_nb, train=True):
        ...
        if train == False:
            noi_free_st_mask, est_targets = self(inputs)
            return self.loss_func(est_targets, targets)
        else:
            alpha = 0.1
            enh_rep, est_targets = self(inputs)
            tgt_1,tgt_2 = torch.split(targets,[1,1],dim=1)
            est_1,est_2 = torch.split(est_targets,[1,1],dim=1)
            enh_rep_tgt= tgt_1 + tgt_2
            # print(enh_rep_tgt.size()) # 4 * 1 * 48000
            enh_rep_tgt= enh_rep_tgt.squeeze(1)
            # print(inputs.size())  # 4 * 48000 
            temp_target = self.model.encoder(enh_rep_tgt)

            ss_loss = self.loss_func(est_targets, targets) 
            se_loss = alpha * self.mse_loss(enh_rep.squeeze(-1), temp_target)

            self.optimizer.zero_grad()
            # self.manual_backward(se_loss)
            se_loss.backward(retain_graph=True)

            se_grads_enc = []
            for i, (key, params) in enumerate(self.model.encoder.named_parameters()):
                se_grads_enc.append(params.grad.data)
            se_grads_enh = []
            for i, (key, params) in enumerate(self.model.masker_enh.named_parameters()):
                se_grads_enh.append(params.grad.data)

            self.optimizer.zero_grad()
            self.manual_backward(ss_loss)
            # ss_loss.backward(retain_graph=True)

            for i, (key, params) in enumerate(self.model.encoder.named_parameters()):
                se_grad = se_grads_enc[i].clone().view(-1)
                ss_grad = params.grad.data.clone().view(-1)
                se_ss_dot = torch.dot(se_grad, ss_grad)
                if se_ss_dot < 0:
                    se_grad = se_grad - se_ss_dot * ss_grad / (ss_grad.norm() ** 2)
                    # se_grad = -se_grad
                params.grad.data = params.grad.data + se_grad.view_as(se_grads_enc[i]).to("cuda:0")

            for i, (key, params) in enumerate(self.model.masker_enh.named_parameters()):
                se_grad = se_grads_enh[i].clone().view(-1)
                ss_grad = params.grad.data.clone().view(-1)
                se_ss_dot = torch.dot(se_grad, ss_grad)
                if se_ss_dot < 0:
                    se_grad = se_grad - se_ss_dot * ss_grad / (ss_grad.norm() ** 2)
                    # se_grad = -se_grad
                params.grad.data = params.grad.data + se_grad.view_as(se_grads_enh[i]).to("cuda:0")
    ...
    def on_validation_epoch_end(self):
        ...
        print("***current lr***",self.optimizer.state_dict()['param_groups'][0]['lr'])
