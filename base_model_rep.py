...
class BaseEncoderMaskerDecoder(BaseModel):
    ...
    """
    #encoder_enhanced
    def __init__(self, encoder,masker_enh,masker_sep, decoder, ssl_model=None, encoder_activation=None):
        super().__init__(sample_rate=getattr(encoder, "sample_rate", None))
        self.encoder = encoder
        self.masker_enh=masker_enh
        self.masker_sep =masker_sep
    ...
    def forward(self, wav):
        ...
        if self.ssl_model!=None:
            with torch.no_grad():
                ssl_feature = self.ssl_model.extract_features(wav.squeeze(1))[0]

        # Real forward
        tf_rep = self.forward_encoder(wav)
        est_masks_enh = self.forward_masker_enh(tf_rep)
        h_e = self.apply_masks(tf_rep, est_masks_enh.squeeze(1),normal=False)
        h_e = h_e.squeeze(1)
        est_masks_sep = self.forward_masker_sep(h_e)
        masked_tf_rep = self.apply_masks(h_e, est_masks_sep,normal=True)
        decoded = self.forward_decoder(masked_tf_rep)

        reconstructed = pad_x_to_y(decoded, wav)
        return h_e,_shape_reconstructed(reconstructed, shape)
    ...
    def forward_masker_enh(self, tf_rep: torch.Tensor,ssl_feature=None) -> torch.Tensor:
        ...
        """
        if ssl_feature== None:
           return self.masker_enh(tf_rep)
        else: 
            return self.masker(tf_rep,ssl_feature)


    def forward_masker_sep(self, tf_rep: torch.Tensor,ssl_feature=None) -> torch.Tensor:
        """Estimates masks from time-frequency representation.
        ...
        """
        if ssl_feature== None:
           return self.masker_sep(tf_rep)
        else: 
            return self.masker(tf_rep,ssl_feature)

    def apply_masks(self, tf_rep: torch.Tensor, est_masks: torch.Tensor,normal=True) -> torch.Tensor:
    ...
    def get_model_args(self):
        ...
        masknet_enh_config = self.masker_enh.get_config()
        masknet_sep_config = self.masker_sep.get_config()
