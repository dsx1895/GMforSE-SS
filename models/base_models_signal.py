...
class BaseEncoderMaskerDecoder(BaseModel):
    ...
    def forward(self, wav):
        ...
        enhanced_wav = self.forward_enh(wav)
        separated_wav = self.forward_sep(enhanced_wav)
    ...
    def forward_enh(self, wav: torch.Tensor) -> torch.Tensor:
        ...
        tf_rep = self.apply_masks(tf_rep, est_masks,sep=False)
        enhanced_wav = self.enh_decoder(tf_rep)
        enhanced_wav = pad_x_to_y(enhanced_wav, wav)
        return _shape_reconstructed(enhanced_wav, shape)
